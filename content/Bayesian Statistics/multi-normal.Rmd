...
$$
\begin{align}
   \mathrm{MultivariateNormal}\pp{\b{A}^{-1}\b{b}, \b{A}^{-1}} \propto \expb{- \frac{1}{2} \bs{\theta}^\top\b{A} \bs{\theta} + \bs{\theta}^\top \b{b}}
\end{align}
$$
...
$$
\boldsymbol{\Sigma} = \frac{1}{n} \mathbf{Z}^\top \mathbf{Z}  
$$
...
$$
\begin{align}
  \mathbf{Z} &\sim \mathcal{N} ( \boldsymbol{0}, \boldsymbol{\phi}_0  ) \\
  \boldsymbol{\Sigma}_0 \mid \boldsymbol{\phi}_0 &\sim \mathbf{Z^\top Z} \\
  &\sim \mathrm{InverseWishart}(\nu_0, \boldsymbol{\phi}_0 )
\end{align}
$$
where $\nu_0$ is a number larger than $k$ (length of sample).
