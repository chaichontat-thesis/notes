...
&\propto (\tst)^{n/2+\alpha-1} \, \expb{-\tst\left[\frac{ns_x^2}{2} + \beta\right]} \\
&\propto \mathrm{Gamma}\left(\frac{n}{2}+\alpha, \frac{ns_x^2}{2} + \beta\right)
\end{align}
$$
If we define $\alpha = \frac{n_0}{2}$ and $\beta=\frac{n_0\sigma_0^2}{2}$, then
$$
p(\tst \mid \xdots, \mu) \sim \pp{\frac{n+n_0}{2}, \frac{ns_x^2+n_0\sigma_0^2}{2}}
$$
...
Let us examine ther second posterior.
$$
\begin{align}
...
&= \mathrm{InverseGamma}\pp{\frac{n_{0,\sigma^2}+n}{2}, \frac{1}{n_{0,\sigma^2}+n}\left[n_{0,\sigma^2}\sigma_0^2+(n-1)s^2+\frac{n_{0,\mu}\, n}{n_{0,\sigma^2}}(\bar{x}-\mu_0)^2\right]}
\end{align}
$$

To recap, the priors and sampling distribution are as followed.
$$
\begin{align}
...
Now we can sample this seauentially to get a joint distribution.